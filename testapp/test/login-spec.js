...
describe('LOGIN', function () {
  before(function (done) {
    ...
    knex.insert([mockData.user2, mockData.user3, mockData.user4]).into('users').then(function (id) {})
    ...
    knex.insert(mockData.perm_factors).into('permissions').then(function (id) {
      ...
      knex.insert([mockData.employee1, mockData.employee2, mockData.employee4]).into('employees').then(function (id) {
        done()
      })
    })
  ...
  describe('/POST login', function () {
    ...
    it(responseMessages.login.userCredentialsTimezone, function (done) {
      chai.request(serverAddress)
        .post(API).send({
          email: 'ewaemployee@celsysemail.celsyswtc.in',
          password: 'Cel@123',
    ...
    it(responseMessages.login.userCredentialsWrongTimezone, function (done) {
      chai.request(serverAddress)
        .post(API).send({
          email: 'ewaemployee@celsysemail.celsyswtc.in',
          password: 'Cel@123',
        ...
        .end((err, res) => {
          if (err) {
            ...
            this.timeout(50000)
            res.body.responseCode.should.equal(4006)
            res.body.message.should.equal(responseMessages.login.wrongTimezone)
            done()
          }
        })
    }).timeout(500000)
